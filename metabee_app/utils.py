...
def update_printers_state_in_db(printers_state: dict):
    for device_id, state in printers_state.items():
        ...
        if state['corrente_A'] >= printer.idle_range and state['corrente_A'] < printer.operating_range:
            Printer.objects.filter(device_id=device_id).update(status=1)
            
        elif state['corrente_A'] >= printer.operating_range:
            Printer.objects.filter(device_id=device_id).update(status=2)
            
        else:
            Printer.objects.filter(device_id=device_id).update(status=0)

def api_state_from_range(printers_state: dict):
    for device_id, state in printers_state.items():
        printer = Printer.objects.filter(device_id=device_id).first()
        
        if state['corrente_A'] >= printer.idle_range and state['corrente_A'] < printer.operating_range:
            return {'name': printer.name, 'state':'ligado'}
            
        elif state['corrente_A'] >= printer.operating_range:
            return {'name': printer.name, 'state':'operando'}
            
        else:
            return {'name': printer.name, 'state':'desligado'}
