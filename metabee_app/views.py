...
from .utils import get_outlet_status, DEVICE_IP, LOCAL_KEY
...
def api_check_printer_on(request, device_id):
    state = get_outlet_status(device_id, DEVICE_IP, LOCAL_KEY)
    if 'erro' in state:
        return JsonResponse({'is_active': False, 'error': state['erro']}, status=500)
    is_active = state.get('corrente_A', 0) > 0.100

    return JsonResponse({'is_active': is_active, 'device_id': device_id})